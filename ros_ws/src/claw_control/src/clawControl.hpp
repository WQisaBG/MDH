...
public:
    ...
    int runWithParameterABC(int slaveId, int pos, int speed, int torque);
     ...
     * */
    int readClawStatus(int slaveId);

protected:
    int safe_modbus_write_raw_req(modbus_t *ctx, uint8_t raw_req[], int raw_req_length);
    int safe_modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *dest);
